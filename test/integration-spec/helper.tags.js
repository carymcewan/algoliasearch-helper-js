...
test('[INT][TAGS]Test tags operations on the helper and their results on the algolia API', function (t) {
  ...
  setup(indexName, function (client, index) {
    return index.addObjects([{ objectID: '0', _tags: ['t1', 't2'] }, { objectID: '1', _tags: ['t1', 't3'] }, { objectID: '2', _tags: ['t2', 't3'] }, { objectID: '3', _tags: ['t3', 't4'] }]).then(function (content) {
    ...
    helper.on('result', function (content) {
      ...
      if (calls === 2) {
        ...
        t.deepEqual(map(content.hits, hitsToParsedID).sort(), [0, 1], 'One tag (t1) expected ids: 0, 1');
        helper.addTag('t2').search();
      }

      if (calls === 3) {
        t.equal(content.hits.length, 1, 'Two tags (t1, t2): 1 result');
        t.deepEqual(map(content.hits, hitsToParsedID).sort(), [0], 'Two tags (t1, t2) expected ids: 0');
        helper.removeTag('t2').toggleTag('t3').toggleTag('t1').search();
      }

      if (calls === 4) {
        t.equal(content.hits.length, 3, 'One tag (t3): 3 results');
        t.deepEqual(map(content.hits, hitsToParsedID).sort(), [1, 2, 3], 'One tag (t3) expected ids: 1, 2, 3');
