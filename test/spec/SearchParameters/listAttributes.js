...
test('removeFacet should remove a facet from the facets list', function (t) {
  ...
  state = SearchParameters.make({}).addFacet('facet').addFacetRefinement('facet', 'value').removeFacet('facet');
...
test('removeDisjunctiveFacet should remove a facet from the disjunctiveFacets list', function (t) {
  var state = SearchParameters.make({}).addDisjunctiveFacet('facet').removeDisjunctiveFacet('facet');

  t.deepEquals(state.disjunctiveFacets, []);

  state = SearchParameters.make({}).addDisjunctiveFacet('facet').addDisjunctiveFacetRefinement('facet', 'value').removeDisjunctiveFacet('facet');
...
test('addHierarchicalFacet should throw when a facet with the same name is already declared', function (t) {
  t.throws(function () {
    SearchParameters.make({ hierarchicalFacets: [{ name: 'facet' }] }).addHierarchicalFacet({ name: 'facet' });
  });
...
test('removeHierarchicalFacet should remove a facet from the hierarchicalFacets list', function (t) {
  var state = SearchParameters.make({}).addHierarchicalFacet({ name: 'facet' }).removeHierarchicalFacet('facet');

  t.deepEquals(state.hierarchicalFacets, []);

  state = SearchParameters.make({}).addHierarchicalFacet({ name: 'facet' }).toggleHierarchicalFacetRefinement('facet', 'value').removeHierarchicalFacet('facet');
