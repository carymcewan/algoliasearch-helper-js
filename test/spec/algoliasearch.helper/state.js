...
test('getState should return the current state of the helper', function (t) {
  ...
  t.deepEquals(helper.getState(), new SearchParameters(initialState), '(getState) getState returned value should be equivalent to initialstate as a new SearchParameters');
  t.deepEquals(helper.getState(), helper.state, '(getState) getState returned value should be equivalent to the internal state of the helper');
...
test('getState should return an object according to the specified filters', function (t) {
  ...
  t.deepEquals(helper.getState(['index', 'query', 'attribute:facetA', 'attribute:facetB', 'attribute:numerical']), stateFinalWithSpecificAttribute, '(getState) getState returned value should contain all the required elements');

  t.deepEquals(helper.getState(['attribute:facetC']), stateWithHierarchicalAttribute, '(getState) getState returned value should contain the hierarchical facet');

  t.deepEquals(helper.getState(['index', 'query', 'attribute:*']), stateFinalWithoutSpecificAttributes, '(getState) getState should return all the attributes if *');
...
test('Get the state as a query string', function (t) {
  ...
  var decodedState = mapKeys(qs.parse(helper.getStateAsQueryString({ filters: filters })), function (v, k) {
    var decodedKey = shortener.decode(k);
    return decodedKey || k;
  });

  t.deepEquals(algoliasearchHelper.SearchParameters._parseNumbers(decodedState), stateWithoutConfig, 'deserialized qs should be equal to the state');
...
test('Set the state with a query parameter with index', function (t) {
  ...
  t.deepEquals(newHelper.state, helper.state, 'Should be able to recreate a helper from a query string');
...
test('Serialize with prefix', function (t) {
  ...
  t.deepEquals(qString, 'toto_q=a%20query&toto_idx=indexNameInTheHelper&toto_dFR[facetB][0]=d&toto_fR[facetA][0]=a&toto_fR[facetWeDontCareAbout][0]=v&toto_nR[numerical][=][0]=3&toto_nR[numerical2][<=][0]=3', 'serialized qs with prefix should be correct');
...
test('Serialize without any state to serialize, only more attributes', function (t) {
  ...
  t.deepEquals(qString, 'toto=tata&foo=bar', 'serialized qs without helper parameters and more attributes should be equal');
...
test('Serialize with prefix, this should have no impact on user provided paramaters', function (t) {
  ...
  t.deepEquals(qString, 'toto_q=a%20query&toto_dFR[facetB][0]=d&toto_fR[facetA][0]=a&toto_fR[facetWeDontCareAbout][0]=v&toto_nR[numerical][=][0]=3&toto_nR[numerical2][<=][0]=3&toto=tata&foo=bar', 'serialized qs with prefix and more attributes should be equal');
...
test('Should be able to deserialize qs with namespaced attributes', function (t) {
  ...
  t.deepEquals(newHelper.state, helper.state, 'Should be able to recreate a helper from a query string (with prefix)');
  t.equal(newHelper.getIndex(), helper.getIndex(), 'Index should be equal even with the prefix');
  t.end();
});
...
test('getStateFromQueryString should parse page as number and be consistent with the state', function (t) {
  ...
  t.deepEquals(partialStateFromQueryString.page, helper.state.page, 'Page should be consistent through query string serialization/deserialization');
  t.end();
});

test('getStateFromQueryString should use its options', function (t) {
  var partialStateFromQueryString = algoliasearchHelper.url.getStateFromQueryString('is_notexistingparam=val&is_MyQuery=test&is_p=3&extra_param=val', {
    prefix: 'is_',
    mapping: {
      q: 'MyQuery'
    }
  ...
  t.deepEquals(partialStateFromQueryString, {
    query: 'test',
    page: 3
