...
test('[Derivated helper] no derivatives', function (t) {
  ...
  function searchTest(requests) {
    t.equal(requests.length, 1, 'Without the derivatives and no filters, the helper generates a single query');
  }
});
...
test('[Derivated helper] 1 derivatives, no modifications', function (t) {
  ...
  function searchTest(requests) {
    ...
    t.deepEqual(requests[0], requests[1], 'the helper generates the same query twice');
  }
});
...
test('[Derivated helper] no derivatives, modification', function (t) {
  ...
  function searchTest(requests) {
    ...
    t.deepEqual(requests[0], requests[1], 'Without the query the other parameters are identical');
  }
});