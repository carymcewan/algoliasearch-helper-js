...
test('[Derivated helper] detach a derivative helper', function (t) {
  ...
  function searchTest(requests) {
    ...
    if (nbRequest === 0) {
      ...
      t.equal(derivedHelper.listeners('result').length, 1, 'one listener is plugged to the derived helper');
      nbRequest++;
    } else if (nbRequest === 1) {
      t.equal(requests.length, 1, 'the helper generates a two queries');
      t.equal(derivedHelper.listeners('result').length, 0, 'no listener on the derived helper');
      t.end();
    }
  }
});