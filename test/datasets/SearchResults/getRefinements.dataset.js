...
if (require.main === module) {
  ...
  var HelperSaver = replayTools.toSaver(Helper, path.join(__dirname.replace('datasets', 'spec'), 'getRefinements'));
  ...
  helper.searchOnce().then(function () {
    ...
    var otherState = initialState.toggleHierarchicalFacetRefinement('hierarchicalCategories', 'Best Buy Gift Cards > Entertainment Gift Cards');
    return helper.searchOnce(otherState);
  }).then(function () {
    helper.__saveLastToFile('hierarchical-cards.json');

    var otherState = initialState.addTagRefinement('foo').addTagRefinement('bar');
    return helper.searchOnce(otherState);
  }).then(function () {
